...
//      std::cout << NGraphPoints << std::endl;
    ...
    for (size_t iTelescope = 0; iTelescope != Event.NTelescopes(); ++iTelescope) {
      ...
      for (size_t iPlane = 0; iPlane != Telescope->NPlanes(); ++iPlane) {
        ...
        if (!hMap_ADC.count(id)) {
          hMap_ADC[id].push_back( new TH1F( TString::Format("Pulse Height ADC for Ch %02i ROC %1i Pixels All", Channel, ROC),
                TString::Format("PulseHeightADC_Ch%02i_ROC%1i_All", Channel, ROC), 100, 150-0.5, 250-0.5) );
          for (size_t ih = 1; ih != 4; ++ih) {
            hMap_ADC[id].push_back( new TH1F( TString::Format("Pulse Height ADC for Ch %02i ROC %1i Pixels %i", Channel, ROC, (int) ih),
                    TString::Format("PulseHeight_Ch%02i_ROC%1i_Pixels%i", Channel, ROC, (int) ih), 100, 150-0.5, 250-0.5) );
          }
        }
        ...
        for (size_t iCluster = 0; iCluster != Plane->NClusters(); ++iCluster) {
          ...
          float const ThisClusterADC    = Cluster->ADC();
          ...
          hMap_ADC[id][0]->Fill( ThisClusterADC );
          if (NHits == 1) {
              hMap_ADC[id][1]->Fill( ThisClusterADC );
          } else if (NHits == 2) {
              hMap_ADC[id][2]->Fill( ThisClusterADC );
          } else if (NHits >= 3) {
              hMap_ADC[id][3]->Fill( ThisClusterADC );
          }
  ...
  TFile* output_root = TFile::Open(TString("plots/pulse_heights_output_" + split_last_part(DataFileName) + ".root"),"RECREATE");
  std::cout << "File created:    " << TString("plots/pulse_heights_output_" + split_last_part(DataFileName) + ".root") << std::endl;
  ...
  for (std::map<int, std::vector<TH1F*> >::iterator it = hMap.begin(); it != hMap.end(); ++it) {
    ...
    h_tempTH1F->SetName("h_summaPulse_ADC");
